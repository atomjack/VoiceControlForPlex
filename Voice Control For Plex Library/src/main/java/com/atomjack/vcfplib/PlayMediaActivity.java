...
import com.atomjack.vcfplib.model.PlexDirectory;
...
import com.atomjack.vcfplib.net.PlexHttpResponseHandler;
...
public class PlayMediaActivity extends Activity implements TextToSpeech.OnInitListener {
	...
	public void onNewIntent(Intent intent) {
		...
		if(from == null) {
			if(origin.equals("GoogleSearchReceiver") || origin.equals("Tasker")) {
				this.queryText = intent.getStringExtra("queryText");
				startup();
			}
    ...
    Boolean next = false;
		...
		if(this.client == null) {
			...
			feedback(getResources().getString(R.string.client_not_found));
			searchDialog.dismiss();
			finish();
			return;
		}
    ...
    Logger.d("showSpecified: %s", showSpecified);
		...
		if(!queryTerm.equals("") || (!episodeSpecified.equals("") && !showSpecified.equals("")) || (!artist.equals("") && (!track.equals("")) || !album.equals(""))) {
          ...
          searchDialog.dismiss();
          feedback(getResources().getString(R.string.nothing_to_play));
          finish();
          return;
        }
		} else {
      ...
      if(server.musicSections.size() == 0) {
        ...
        if(serversSearched == plexmediaServers.size()) {
          if(albums.size() == 1) {
            ...
            feedback(getResources().getString(albums.size() > 1 ? R.string.found_more_than_one_album : R.string.couldnt_find_album));
            searchDialog.dismiss();
            finish();
            return;
          }
...
//              if(thisAlbum.title.toLowerCase().equals(album.toLowerCase()) || artist.equals("")) {
	...
	private void searchForSong(final String artist, final String track) {
      ...
      server.musicSectionsSearched = 0;
            ...
            searchDialog.dismiss();
						...
						feedback(getResources().getString(R.string.couldnt_find_track));
						searchDialog.dismiss();
						finish();
						return;
					}
	...
	private void playSpecificEpisode(String showSpecified) {
		if(videos.size() == 0) {
			feedback(getResources().getString(R.string.couldnt_find_episode));
			searchDialog.dismiss();
			finish();
			return;
      ...
      if(!exactMatch) {
        feedback(getResources().getString(R.string.found_more_than_one_show));
        searchDialog.dismiss();
        finish();
        return;
      }
    ...
    serversSearched = 0;
        ...
        serversSearched++;
				feedback(getResources().getString(R.string.couldnt_find), queryTerm);
				searchDialog.dismiss();
				finish();
				return;
        ...
        PlexHttpClient.get(url, null, new PlexHttpMediaContainerHandler()
        {
          ...
          public void onSuccess(MediaContainer mc)
          {
            ...
            if(foundSeason == null && serversSearched == plexmediaServers.size() && !videoPlayed) {
              serversSearched++;
              feedback(getResources().getString(R.string.couldnt_find_season));
              searchDialog.dismiss();
              finish();
              return;
  ...
  private void doLatestEpisode(final String queryTerm) {
	...
	private void onMovieSearchFinished(String queryTerm) {
      ...
      if(!exactMatch) {
        feedback(getResources().getString(R.string.found_more_than_one_movie));
        searchDialog.dismiss();
        finish();
        return;
      }
  ...
  public void feedback(String text) {
	...
	public void onInit(int status) {
		mTts.setLanguage(VoiceControlForPlexApplication.getVoiceLocale(mPrefs.getString(VoiceControlForPlexApplication.PREF_FEEDBACK_VOICE, "Locale.US")));
